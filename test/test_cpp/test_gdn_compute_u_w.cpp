/*
 * GDN compute_u_w Kernel Testing Harness
 *
 * Tests the sm90_bf16_compute_u_w kernel which computes:
 *   A = tril(diag(beta) * K * K^T)
 *   T = (I - A)^{-1} via forward substitution
 *   U = T * V
 *   W = T * K  (optionally scaled by exp(gate))
 */
...
static const std::vector<ChunkedTestShape> test_shapes = {
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> compute_u_w_chunk_reference(
    const torch::Tensor& K, const torch::Tensor& V, const torch::Tensor& beta,
    const std::optional<torch::Tensor>& gate = std::nullopt) {
    int64_t chunk_size = K.size(0);
    auto K_f = K.to(torch::kFloat32);
    auto V_f = V.to(torch::kFloat32);
    auto beta_f = beta.to(torch::kFloat32);

    auto KKT = torch::mm(K_f, K_f.t());
    auto A = torch::tril(KKT, -1);

    if (gate.has_value()) {
        auto g = gate->to(torch::kFloat32);
        A = A * torch::exp(g.unsqueeze(1) - g.unsqueeze(0));
    }
    A = A * beta_f.unsqueeze(1);

    auto options = torch::TensorOptions().dtype(torch::kFloat32).device(K.device());
    auto T = torch::inverse(A + torch::eye(chunk_size, options));
    T = T * beta_f.unsqueeze(0);

    auto U = torch::mm(T, V_f);
    auto W = torch::mm(T, K_f);
    if (gate.has_value()) {
        auto g = gate->to(torch::kFloat32);
...
torch::Tensor compute_u_w_reference_padded(
    ...
    int64_t nvh = V.size(2);
    torch::Tensor U = torch::zeros_like(V);
    torch::Tensor W = torch::zeros({B, T, nvh, K.size(3)}, V.options());

    int64_t num_chunks = (T + chunk_size - 1) / chunk_size;
    for (int64_t b = 0; b < B; ++b) {
        for (int64_t h = 0; h < nvh; ++h) {
            ...
            for (int64_t c = 0; c < num_chunks; ++c) {
                ...
                auto [U_c, W_c, _] = compute_u_w_chunk_reference(K_c, V_c, b_c, g_c);
                U[b].slice(0, cs, ce).select(1, h).copy_(U_c);
                W[b].slice(0, cs, ce).select(1, h).copy_(W_c);
            }
        }
    }
    return U;  // We only check U for brevity (W check omitted for now)
}
...
bool test_padded_mode(const ChunkedTestShape& shape, const TestConfig& config) {
    ...
    try {
        ...
        float sk = 1.0f / std::sqrt((float)shape.shape_k);
        float sv = 1.0f / std::sqrt((float)shape.shape_v);
        ...
        torch::Tensor beta = torch::sigmoid(
            torch::randn({shape.batch_size, shape.seq_len, shape.num_v_heads}, bf16_opts) * 0.3f);
        ...
        if (config.use_gate) {
            gate_f32 =
                torch::randn({shape.batch_size, shape.seq_len, shape.num_v_heads}, f32_opts) * 0.1f;
        ...
        // Reference U
        torch::Tensor U_ref =
            compute_u_w_reference_padded(K, V, beta, shape.chunk_size, gate_ref_opt);

        // Kernel
        torch::Tensor U_kernel = torch::zeros_like(V);
        torch::Tensor W_kernel = torch::zeros(
            {shape.batch_size, shape.seq_len, shape.num_v_heads, shape.shape_k}, bf16_opts);

        cudaStream_t stream;
        CUDA_CHECK(cudaStreamCreate(&stream));

        torch::Tensor beta_mn = gdn_cuda::transpose_to_mn_major(beta, stream, 16);

        std::optional<torch::Tensor> gate_mn_opt = std::nullopt;
        if (config.use_gate) {
            torch::Tensor g_mn = gdn_cuda::transpose_to_mn_major(gate_cumsum_f32, stream, 16);
            gate_mn_opt = g_mn;
        }
        ...
        gdn_cuda::bf16_gdn_compute_u_w(K, V, U_kernel, W_kernel, beta_mn, gate_mn_opt, "t", stream,
                                       cu_seqlens_opt, chunk_indices_opt);
        CUDA_CHECK(cudaStreamSynchronize(stream));

        std::cout << "Checking U:\n";
        bool passed = check_tensor_close(U_ref, U_kernel, config.atol, config.rtol);
...
torch::Tensor compute_u_w_reference_varlen(
    const torch::Tensor& K, const torch::Tensor& V, const torch::Tensor& beta, int64_t chunk_size,
    const std::vector<int>& cu_seqlens, const std::optional<torch::Tensor>& gate = std::nullopt) {
    int64_t nkh = K.size(1), nvh = V.size(1);
    ...
    for (int64_t b = 0; b < batch_size; ++b) {
        ...
        for (int64_t h = 0; h < nvh; ++h) {
            ...
            for (int64_t c = 0; c < nc; ++c) {
                ...
                auto [U_c, W_c, _] = compute_u_w_chunk_reference(K_c, V_c, b_c, g_c);
                U.slice(0, cs, ce).select(1, h).copy_(U_c);
            }
        }
    }
    return U;
...
bool test_varlen_mode(const VarlenTestShape& shape, const TestConfig& config) {
    ...
    try {
        ...
        torch::Tensor K = torch::normal(0, sk, {total_tokens, shape.num_k_heads, shape.shape_k},
                                        std::nullopt, bf16_opts);
        torch::Tensor V = torch::normal(0, sv, {total_tokens, shape.num_v_heads, shape.shape_v},
                                        std::nullopt, bf16_opts);
        torch::Tensor beta =
            torch::sigmoid(torch::randn({total_tokens, shape.num_v_heads}, bf16_opts) * 0.3f);

        torch::Tensor gate_f32;
        torch::Tensor gate_cumsum_f32;
        std::optional<torch::Tensor> gate_ref_opt = std::nullopt;
        if (config.use_gate) {
            gate_f32 = torch::randn({total_tokens, shape.num_v_heads}, f32_opts) * 0.1f;
            gate_cumsum_f32 =
        ...
        torch::Tensor U_ref =
            compute_u_w_reference_varlen(K, V, beta, shape.chunk_size, cu_seqlens, gate_ref_opt);

        torch::Tensor U_kernel = torch::zeros_like(V);
        torch::Tensor W_kernel =
            torch::zeros({total_tokens, shape.num_v_heads, shape.shape_k}, bf16_opts);
        ...
        std::optional<torch::Tensor> gate_mn_opt = std::nullopt;
        if (config.use_gate) {
            gate_mn_opt = gdn_cuda::transpose_to_mn_major(gate_cumsum_f32, stream, 16);
        }
...
int main(int argc, char** argv) {
    ...
    std::cout << "============================================\n";
    std::cout << "GDN compute_u_w Testing Harness\n";
    std::cout << "============================================\n";
    ...
    if (config.test_padded) {
        ...
        for (const auto& shape : test_shapes) {
            bool r = test_padded_mode(shape, config);
            if (r) {
    ...
    if (config.test_varlen) {
        ...
        for (const auto& shape : varlen_test_shapes) {
            bool r = test_varlen_mode(shape, config);
            if (r) {
        ...
        for (const auto& shape : varlen_partial_test_shapes) {
            bool r = test_varlen_partial_mode(shape, config);
            if (r) {
                std::cout << "\033[0;32m[PASSED] Varlen Partial: " << shape.description
                          << "\033[0m\n";
                ++passed;
            } else {
                std::cout << "\033[0;31m[FAILED] Varlen Partial: " << shape.description
                          << "\033[0m\n";
                ++failed;
            }
        }
