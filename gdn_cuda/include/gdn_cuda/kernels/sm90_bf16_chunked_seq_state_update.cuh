...
#pragma unroll
                ...
                if constexpr (kUseGate) {
// now that V is fully updated, we multiply the V with the gate values
#pragma unroll
                    for (int val_idx = 0; val_idx < WSMMA::kNumAccum; val_idx++) {
                        auto [row_idx, col_idx] = get_accum_row_col(threadIdx.x, val_idx);
                        bool pred = global_row_offset + row_idx < seq_end;
                        if (pred) {
                            ws_accum[val_idx] *=
                                __expf(gate_last - s_gate[row_idx]);  // gating decay
                        }
                    }
...
#pragma unroll
                    ...
                    // Wait for the TMA store of pre-gating sU to finish reading smem
                    // before we overwrite
                    if (warpIdx == 0) {
                        cute::tma_store_wait<0>();
                    }
                    ...
                    // Write gated ws_accum back to sU so the outer SS WGMMA reads gated
                    // values
                    store_accum_to_swizzled_smem<BLOCK_V, kChunkSize, kSwizzleUMode,
                                                 WGMMA_M_PER_WARP>(ws_accum, sU, warpIdx, lane_idx);
                    cutlass::arch::NamedBarrier::sync(kNumMathThreads, 1);
                }
                ...
                if constexpr (kIsVarLen && kOutputFinalState) {
                    ...
                    if (!full_chunk) {
                        ...
                        load_swizzled_smem_to_accum<SHAPE_K, kChunkSize, kSwizzleKMode,
                                                    WGMMA_M_PER_WARP>(sK, sK_accum, warpIdx,
                                                                      lane_idx);
#pragma unroll
                        ...
                        store_accum_to_swizzled_smem<SHAPE_K, kChunkSize, kSwizzleKMode,
                                                     WGMMA_M_PER_WARP>(sK_accum, sK, warpIdx,
                                                                       lane_idx);
                        cutlass::arch::NamedBarrier::sync(kNumMathThreads, 1);
                    }
                }
